...
class MultiKMeans:
    ...
    def fit_predict(self, X):
        ...
        # print(np.random.choice(batch_size, size=[self.n_clusters], replace=False).shape)
        ...
        #     X = X.detach().numpy()
        #     self.centroids = self.centroids.detach().numpy()
        #     # print(X.shape, self.centroids.shape, closest.shape)
        #     for idx, (c, x) in enumerate(zip(self.centroids, X)):
        #         plt.scatter(x[:, 0], x[:, 1], c=closest[idx], marker='.', cmap='hsv')
        #         plt.scatter(c[:, 0], c[:, 1], c=list(range(20)), marker='o', s=100, cmap='hsv')
        #         plt.show()
        # END SCATTER
        # if self.verbose >= 1:
        #     print(
        #         f'used {i + 1} iterations ({round(time() - start_time, 4)}s) to cluster {self.n_kmeans}x{batch_size} items into {self.n_clusters} clusters')
