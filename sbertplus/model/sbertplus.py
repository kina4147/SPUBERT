...
from SocialBERT.sbertplus.model.modeling_sbert import (
    SBertEncoder,
    SBertModelBase,
)
...
from SocialBERT.sbertplus.model.embedding import SBertEmbeddings, SBertMMEmbeddings
from SocialBERT.sbertplus.model.cvae import (
    GoalRecognitionNet, GoalPriorNet, GoalEncoder, TrajGRUEncoder, TrajMLPEncoder, TrajHiddenGRUEncoder)
from SocialBERT.sbertplus.model.decoder import SpatialDecoder, GoalDecoder, SIPDecoder, MTPDecoder
from SocialBERT.sbertplus.model.pooler import GoalPooler, FullTrajPooler, PastTrajPooler, FutureTrajPooler, MTPPooler, SIPPooler
from SocialBERT.sbertplus.model.loss import (
...
from SocialBERT.sbertplus.model.kmean_sampler import MultiKMeans
...
class SBertPlusModel(SBertModelBase):
    def __init__(self, cfgs):
        super().__init__(cfgs)
        if self.cfgs.scene:
            self.embeddings = SBertMMEmbeddings(cfgs)
        else:
...
class SBertPlusPTOutput(OrderedDict):
...
class SBertPlusTGPOutput(OrderedDict):
    total_loss: torch.FloatTensor = None
...
class SBertPlusMGPOutput(OrderedDict):
    total_loss: torch.FloatTensor = None
    col_loss: torch.FloatTensor = None
...
class SBertPlusMGPModel(SBertModelBase):
    def __init__(self, goal_cfgs, share_enc=None):
        ...
        if share_enc:
            ...
            self.sbert = SBertPlusModel(goal_cfgs)
...
class SBertPlusFTOutput(OrderedDict):
    total_loss: torch.FloatTensor = None
    gde_loss: torch.FloatTensor = None
    ade_loss: torch.FloatTensor = None
    fde_loss: torch.FloatTensor = None
...
class SBertPlusFTModel(SBertModelBase):
    ...
    def forward(
        ...
        # tgp_spatial_ids = self.add_best_goals(spatial_ids=mgp_spatial_ids.clone(), pred_goals=mgp_out["pred_goals"],
        #                                       best_idx=mgp_out["best_idx"], mask_val=self.cfgs.view_range,
        #                                       pad_val=-self.cfgs.view_range)
        #
        # best_pred_goals = mgp_out["pred_goals"][range(len(mgp_out["best_idx"])), mgp_out["best_idx"]]
        ...
        # kld_loss = kld_weight * mgp_out["kld_loss"]
        # gde_loss = goal_weight * mgp_out["gde_loss"]
        # ade_loss = traj_weight * tgp_out["ade_loss"]
        # fde_loss = traj_weight * tgp_out["fde_loss"]
        # if self.cfgs.scene:
        #     mgp_col_loss = self.cfgs.col_weight * mgp_out["col_loss"]
        #     tgp_col_loss = self.cfgs.col_weight * tgp_out["col_loss"]
        #     total_loss = kld_loss + gde_loss + mgp_col_loss + ade_loss + fde_loss + mgp_col_loss
        # else:
        #     mgp_col_loss = None
        #     tgp_col_loss = None
        #     total_loss = kld_loss + gde_loss + ade_loss + fde_loss
        ...
        # else:
        #     mgp_out = self.mgp_model(spatial_ids=spatial_ids, segment_ids=mgp_segment_ids,
        #                              temporal_ids=mgp_temporal_ids, attn_mask=mgp_attn_mask,
        #                              traj_lbl=traj_lbl, goal_lbl=goal_lbl, output_attentions=output_attentions,
        #                              kld_weight=kld_weight)
        #     tgp_spatial_ids = self.add_best_goals(spatial_ids=spatial_ids.clone(), pred_goals=mgp_out["pred_goals"],
        #                                           best_idx=mgp_out["best_idx"], mask_val=self.cfgs.view_range,
        #                                           pad_val=-self.cfgs.view_range)
        #
        #     tgp_out = self.tgp_model(
        #         spatial_ids=tgp_spatial_ids, segment_ids=tgp_segment_ids, temporal_ids=tgp_temporal_ids,
        #         attn_mask=tgp_attn_mask, traj_lbl=traj_lbl, output_attentions=output_attentions)
        #     kld_loss = kld_weight * mgp_out["kld_loss"]
        #     gde_loss = goal_weight * mgp_out["gde_loss"]
        #     traj_ade_loss = traj_weight * tgp_out["traj_ade_loss"]
        #     traj_fde_loss = traj_weight * mgp_tgp_fde_loss(mgp_out["pred_goals"][range(len(mgp_out["best_idx"])), mgp_out["best_idx"]], tgp_out["pred_trajs"])
        #     total_loss = kld_loss + gde_loss + traj_ade_loss + traj_fde_loss
        #     return SBertPlusFinetuningOutput(
        #         total_loss=total_loss,
        #         traj_ade_loss=traj_ade_loss,
        #         traj_fde_loss=traj_fde_loss,
        #         gde_loss=gde_loss,
        #         kld_loss=kld_loss,
        #         pred_trajs=tgp_out["pred_trajs"],
        #         pred_goals=mgp_out["pred_goals"],
...
# class SBertPlusFinetuningModel(SBertModelBase):
#
#     def __init__(self, tgp_cfgs, mgp_cfgs, cfgs):
#         super().__init__(cfgs)
#
...
#
#
#         # k_scale = scale.unsqueeze(1).repeat(1, self.cfgs.k_sample, 1)
#         if self.cfgs.scale:
#             spatial_ids[:, :, 1 + self.cfgs.obs_len:self.cfgs.obs_len + self.cfgs.pred_len, :] = mask_val/self.cfgs.view_range
#             # goals_st = goals / self.cfgs.view_range
#             spatial_ids[:, :, self.cfgs.obs_len + self.cfgs.pred_len, :self.cfgs.goal_dim] = goals / self.cfgs.view_range
#             spatial_ids[:, :, self.cfgs.obs_len + self.cfgs.pred_len, self.cfgs.goal_dim:] = pad_val/self.cfgs.view_range
#         else:
#             spatial_ids[:, :, 1 + self.cfgs.obs_len:self.cfgs.obs_len + self.cfgs.pred_len, :] = mask_val
#             # spatial_ids[:, :, 1 + self.cfgs.obs_len:self.cfgs.obs_len + self.cfgs.pred_len, :] /= k_scale.unsqueeze(2)
#             # goals_st = goals/k_scale
#             spatial_ids[:, :, self.cfgs.obs_len + self.cfgs.pred_len, :self.cfgs.goal_dim] = goals
#             spatial_ids[:, :, self.cfgs.obs_len + self.cfgs.pred_len, self.cfgs.goal_dim:] = pad_val
#         # spatial_ids[:, :, self.cfgs.obs_len + self.cfgs.pred_len, :] /= k_scale
#         return spatial_ids
#
#     def forward(
#             self,
...
#             env_batch_size, env_seq_len, env_spatial_dim = env_spatial_ids.size()
#             mgp_out = self.mgp_model(
#                 train=train, spatial_ids=spatial_ids, segment_ids=mgp_segment_ids, temporal_ids=mgp_temporal_ids, attn_mask=mgp_attn_mask,
#                 env_spatial_ids=env_spatial_ids, env_temporal_ids=env_temporal_ids, env_segment_ids=env_segment_ids,
#                 env_attn_mask=env_attn_mask, traj_lbl=traj_lbl, goal_lbl=goal_lbl, output_attentions=output_attentions, kld_weight=kld_weight)
#             k_goal_spatial_ids = self.add_goals(spatial_ids, mgp_out["pred_goals"], mask_val=self.cfgs.view_range, pad_val=-self.cfgs.view_range).view(-1, traj_seq_len, self.cfgs.input_dim)
#             k_goal_segment_ids = tgp_segment_ids.unsqueeze(1).repeat(1, self.cfgs.k_sample, 1).view(-1, traj_seq_len)  # B * K * Sdim -> BK * Seq
#             k_goal_temporal_ids = tgp_temporal_ids.unsqueeze(1).repeat(1, self.cfgs.k_sample, 1).view(-1, traj_seq_len)  # B * K * Sdim -> BK * Seq
#             k_goal_attn_mask = tgp_attn_mask.unsqueeze(1).repeat(1, self.cfgs.k_sample, 1).view(-1, traj_seq_len)  # B * K * Sdim -> BK * Seq
#             k_goal_env_spatial_ids = env_spatial_ids.unsqueeze(1).repeat(1, self.cfgs.k_sample, 1, 1).view(-1, env_seq_len, env_spatial_dim)
...
#             k_goal_env_attn_mask = env_attn_mask.unsqueeze(1).repeat(1, self.cfgs.k_sample, 1).view(-1, env_seq_len)
#             if train:
#                 k_goal_traj_lbs = traj_lbs.unsqueeze(1).repeat(1, self.cfgs.k_sample, 1, 1).view(-1, self.cfgs.pred_len, self.cfgs.output_dim)
#
#                 tgp_out = self.tgp_model(
#                     train=train, spatial_ids=k_goal_spatial_ids, segment_ids=k_goal_segment_ids, temporal_ids=k_goal_temporal_ids,
#                     attn_mask=k_goal_attn_mask, env_spatial_ids=k_goal_env_spatial_ids, env_segment_ids=k_goal_env_segment_ids,
#                     env_temporal_ids=k_goal_env_temporal_ids, env_attn_mask=k_goal_env_attn_mask, traj_lbs=k_goal_traj_lbs, output_attentions=output_attentions)
#
#                 pred_trajs = tgp_out["pred_trajs"].reshape(traj_batch_size, self.cfgs.k_sample, self.cfgs.pred_len, self.cfgs.output_dim)
#                 pred_goals = mgp_out["pred_goals"].reshape(traj_batch_size, self.cfgs.k_sample, self.cfgs.goal_dim)  # BK * S => B*K*S
#                 # total_loss = mgp_out["total_loss"] + tgp_out["total_loss"]
#                 # gde_loss, traj_ade_loss, traj_fde_loss = tgp_cvae_loss(pred_goals, pred_trajs, goal_lbs, traj_lbs, self.cfgs.k_sample, True)
#                 goal_col_loss = self.cfgs.col_weight*goal_collision_loss(pred_goals, envs, envs_params)
#                 traj_col_loss = self.cfgs.col_weight*pos_collision_loss(pred_trajs, envs, envs_params)
#                 traj_ade_loss, traj_fde_loss = tgp_cvae_loss(mgp_out["best_idx"], pred_goals, pred_trajs, goal_lbs, traj_lbs, self.cfgs.k_sample,
#                                                              goal_dim=self.cfgs.goal_dim, output_dim=self.cfgs.output_dim, best_of_many=True)
#                 kld_loss = kld_weight * mgp_out["kld_loss"]
#                 gde_loss = goal_weight * mgp_out["gde_loss"] # self.cfgs.goal_weight * gde_loss
#                 traj_ade_loss = traj_weight * traj_ade_loss # self.cfgs.traj_weight * traj_ade_loss
#                 traj_fde_loss = traj_weight * traj_fde_loss # self.cfgs.goal_weight * traj_fde_loss
#                 total_loss = kld_loss + gde_loss + traj_ade_loss + traj_fde_loss # + traj_col_loss + goal_col_loss
#                 return SBertPlusFinetuningOutput(
#                     total_loss=total_loss,
#                     traj_ade_loss=traj_ade_loss,#tgp_out["traj_ade_loss"],
#                     traj_fde_loss=traj_fde_loss,#tgp_out["traj_fde_loss"],
#                     gde_loss=gde_loss,#mgp_out["gde_loss"],
#                     traj_col_loss=traj_col_loss,
#                     goal_col_loss=goal_col_loss,
#                     kld_loss=kld_loss,
#                     pred_trajs=pred_trajs,
#                     pred_goals=pred_goals,
#                     goal_attentions=mgp_out["attentions"],
...
#
#         else:
#             mgp_out = self.mgp_model(train=train, spatial_ids=spatial_ids, segment_ids=mgp_segment_ids, temporal_ids=mgp_temporal_ids, attn_mask=mgp_attn_mask,
#                            traj_lbs=traj_lbs, goal_lbs=goal_lbs, output_attentions=output_attentions, kld_weight=kld_weight)
#
#             k_goal_spatial_ids = self.add_goals(spatial_ids, mgp_out["pred_goals"], mask_val=self.cfgs.view_range, pad_val=-self.cfgs.view_range).view(-1, traj_seq_len, self.cfgs.input_dim)
#             k_goal_segment_ids = tgp_segment_ids.unsqueeze(1).repeat(1, self.cfgs.k_sample, 1).view(-1, traj_seq_len)  # B * K * Sdim -> BK * Seq
#             k_goal_temporal_ids = tgp_temporal_ids.unsqueeze(1).repeat(1, self.cfgs.k_sample, 1).view(-1, traj_seq_len)  # B * K * Sdim -> BK * Seq
...
#                 tgp_out = self.tgp_model(
#                     train=train, spatial_ids=k_goal_spatial_ids, segment_ids=k_goal_segment_ids, temporal_ids=k_goal_temporal_ids,
#                     attn_mask=k_goal_attn_mask, traj_lbs=k_goal_traj_lbs, output_attentions=output_attentions)
#                 pred_trajs = tgp_out["pred_trajs"].reshape(traj_batch_size, self.cfgs.k_sample, self.cfgs.pred_len, self.cfgs.output_dim)
#                 # total_loss = mgp_out["total_loss"] + tgp_out["total_loss"]
#                 pred_goals = mgp_out["pred_goals"].reshape(traj_batch_size, self.cfgs.k_sample, self.cfgs.goal_dim)  # BK * S => B*K*S                # total_loss = mgp_out["total_loss"] + tgp_out["total_loss"]
#                 traj_ade_loss, traj_fde_loss = tgp_cvae_loss(mgp_out["best_idx"], pred_goals, pred_trajs, goal_lbs, traj_lbs, self.cfgs.k_sample, True)
#                 kld_loss = kld_weight * mgp_out["kld_loss"]
#                 gde_loss = goal_weight * mgp_out["gde_loss"] # self.cfgs.goal_weight * gde_loss
#                 traj_ade_loss = traj_weight * traj_ade_loss # self.cfgs.traj_weight * traj_ade_loss
#                 traj_fde_loss = traj_weight * traj_fde_loss # self.cfgs.goal_weight * traj_fde_loss
...
#                 tgp_out = self.tgp_model(
#                     train=train, spatial_ids=k_goal_spatial_ids, segment_ids=k_goal_segment_ids, temporal_ids=k_goal_temporal_ids,
#                     attn_mask=k_goal_attn_mask, output_attentions=output_attentions)
#                 pred_trajs = tgp_out["pred_trajs"].reshape(traj_batch_size, self.cfgs.k_sample, self.cfgs.pred_len, self.cfgs.output_dim)
#                 pred_goals = mgp_out["pred_goals"].reshape(traj_batch_size, self.cfgs.k_sample, self.cfgs.goal_dim)  # BK * S => B*K*S
#                 return SBertPlusFinetuningOutput(
#                     pred_trajs=pred_trajs,
