...
def viz_input_trajectory(input_trajs, obs_len=8, pred_len=12, num_nbr=4, view_range=20.0, view_angle=np.pi/3, social_range=2.0, pad_val=-20, ax=None,
              ...
              zorder=0, viz_interest_area=False, arrow=False, arrow_zorder=None):
    trajs = copy.deepcopy(input_trajs)
    if ax is None:
        fig, ax = plt.subplots()

    if arrow_zorder is None:
        arrow_zorder = zorder + 5

    sep_trajs = separate_trajs(trajs, obs_len, pred_len, num_nbr, pad_val)
    tgt_traj = sep_trajs[0]
    ax.scatter(tgt_traj[:obs_len-1, 0], tgt_traj[:obs_len-1, 1], c='red', edgecolors='k', zorder=arrow_zorder-1)
    ax.scatter(tgt_traj[obs_len-1, 0], tgt_traj[obs_len-1, 1], c='red', edgecolors='k', marker='o', zorder=arrow_zorder+1)
    ax.plot(tgt_traj[:obs_len, 0], tgt_traj[:obs_len, 1], c='red', alpha=0.5, zorder=arrow_zorder-2)
    ...
    if arrow:
        ax.arrow(tgt_traj[obs_len-1, 0], tgt_traj[obs_len-1, 1], dx, dy, color='k',
                 width=arrow_width, head_width=dist_scale, head_length=dist_scale, zorder=arrow_zorder)

    if viz_interest_area:
        ax.add_patch(mpl.patches.Wedge((0, 0), r=view_range, theta1=-view_angle*90/np.pi, theta2=view_angle*90/np.pi, alpha=0.5, color='k', fill=False, linestyle='--', zorder=0))
        ax.add_patch(mpl.patches.Circle((0, 0), radius=social_range, alpha=0.5, color='k', fill=False, linestyle='--', zorder=0))
    ...
    for idx, traj in enumerate(sep_trajs[1:]):
        ...
        ax.scatter(traj[obs_len-1, 0], traj[obs_len-1, 1], c=nbr_colors[idx], edgecolors='k', marker='o', zorder=arrow_zorder+1)
        ax.plot(traj[:obs_len, 0], traj[:obs_len, 1], c=nbr_colors[idx], alpha=0.5, zorder=zorder)
...
def viz_gt_trajs(input_traj, alpha=0.2, dot=False, scale=1, linewidth=1, color="yellow", ax=None, zorder=0):
    traj = copy.deepcopy(input_traj)
    if ax is None:
        fig, ax = plt.subplots()
    if dot:
        s = scale * mpl.rcParams['lines.markersize'] ** 2
        ax.scatter(traj[:, 0], traj[:, 1], alpha=alpha, c=color, s=s, edgecolors='k', zorder=zorder+1)
        ax.plot([0, traj[0, 0]], [0, traj[0, 1]], c=color, alpha=alpha, zorder=zorder)
        ax.plot(traj[:, 0], traj[:, 1], c=color, alpha=alpha, zorder=zorder)
    else:
        ax.plot([0, traj[0, 0]], [0, traj[0, 1]], c=color, alpha=alpha, linewidth=linewidth, zorder=zorder+1)
        ax.plot(traj[:, 0], traj[:, 1], c=color, alpha=alpha, linewidth=linewidth, zorder=zorder+1)
        ax.plot([0, traj[0, 0]], [0, traj[0, 1]], alpha=alpha, c='k', linewidth=linewidth*1.5, zorder=zorder)
        ax.plot(traj[:, 0], traj[:, 1], alpha=alpha, c='k', linewidth=linewidth*1.5, zorder=zorder)


def viz_pred_trajs(input_traj, alpha=0.2, dot=False, color="red", linewidth=1, ax=None, zorder=0):
    traj = copy.deepcopy(input_traj)
    traj = input_traj
    if ax is None:
        fig, ax = plt.subplots()
    if dot:
        ...
        ax.plot([0, traj[0, 0]], [0, traj[0, 1]], alpha=alpha, c=color, zorder=zorder)
        ax.plot(traj[:, 0], traj[:, 1], c=color, alpha=alpha, zorder=zorder)
    # ax.scatter(traj[:, 0], traj[:, 1], c='yellow', alpha=alpha, edgecolors='k', zorder=zorder)
    else:
        ax.plot([0, traj[0, 0]], [0, traj[0, 1]], alpha=alpha, c=color, linewidth=linewidth, zorder=zorder+1)
        ax.plot(traj[:, 0], traj[:, 1], alpha=alpha, c=color, linewidth=linewidth, zorder=zorder+1)
        ax.plot([0, traj[0, 0]], [0, traj[0, 1]], alpha=alpha, c='k', linewidth=linewidth*1.5, zorder=zorder)
        ax.plot(traj[:, 0], traj[:, 1], alpha=alpha, c='k', linewidth=linewidth*1.5, zorder=zorder)
...
def viz_goal_samples(input_samples, alpha=0.5, ax=None, color='blue', zorder=0):
    if ax is None:
        fig, ax = plt.subplots()
    ax.scatter(input_samples[:, 0], input_samples[:, 1], c=color, edgecolor="none", marker='.', alpha=alpha, zorder=zorder)


def viz_gt_goal(input_goal, ax=None, color='yellow', alpha=1.0, scale=None, zorder=0):
    goal = copy.deepcopy(input_goal)
    if ax is None:
        fig, ax = plt.subplots()
    if scale is None:
        ax.scatter(goal[0], goal[1], c=color, alpha=alpha, marker='X', edgecolor='k', zorder=zorder)
    else:
        s = scale * mpl.rcParams['lines.markersize'] ** 2
        ax.scatter(goal[0], goal[1], c=color, alpha=alpha, marker='X', s=s, edgecolor='k', zorder=zorder)
...
def viz_k_pred_goals(input_goals, alpha=0.5, scale=None, color='blue', ax=None, zorder=0):
    goals = copy.deepcopy(input_goals)
    if ax is None:
        fig, ax = plt.subplots()
    if scale is None:
        ax.scatter(goals[:, 0], goals[:, 1], c=color, edgecolor='none', marker='X', alpha=alpha, zorder=zorder)
    else:
        s = scale * mpl.rcParams['lines.markersize'] ** 2
        ax.scatter(goals[:, 0], goals[:, 1], c=color, marker='X', s=s, edgecolor='none', alpha=alpha, zorder=zorder)
...
def viz_trajectory_attention(input_trajs, input_gt_trajs, input_pred_trajs, input_traj_attentions, self_attn=True, env_seq_len=None, layer_id=0, head_id=0,
    ...
    if max_attn:
        ...
        if self_attn:
            ...
            tgt_traj[tgt_traj == pad_val] = np.nan
            # scale = self_attn * 3000
            # self_attn = self_attn / max(self_attn)
            # ax.scatter(tgt_traj[:obs_len, 0], tgt_traj[:obs_len, 1], marker='o', c='red', s=scale[:obs_len])
        ...
        # nbr_attn = attn[nbr_ids, :]
        # nbr_attn = nbr_attn[:, tgt_ids]
        # nbr_attn = nbr_attn.squeeze()
        # # nbr_attn = nbr_attn / np.max(nbr_attn)
        # src_tgt_traj = tgt_traj[tgt_ids[:obs_len+pred_len]]
        # for src_tpidx, src_tgt_pos in enumerate(src_tgt_traj):
        #     for nbr_idx, nbr_traj in enumerate(sep_trajs[1:]):
        #         for nbr_pos_idx, nbr_pos in enumerate(nbr_traj):
        ...
        #                     alpha=nbr_attn[nbr_attn_idx, src_tpidx], c=nbr_colors[nbr_idx], zorder=5)
...
def viz_attention(attentions, obs_len=8, pred_len=12, num_nbr=2, head_id=0, env_seq_len=None, ax=None, order=0):
    num_head = 4
    if ax is None:
        fig, ax = plt.subplots()
    ...
    for i in range(num_head):
        ...
        tgt_ids = np.zeros(traj_seq_len + env_seq_len, dtype=bool)
        tgt_ids[:1 + obs_len + pred_len] = True
        nbr_ids = np.ones(traj_seq_len + env_seq_len, dtype=bool)
        nbr_ids[:1 + obs_len + pred_len] = False  # Target Trajectory
        nbr_ids[1 + obs_len + pred_len::(1 + obs_len)] = False  # SEP
        nbr_ids[traj_seq_len:] = False  # Environment
        env_ids = np.zeros(traj_seq_len + env_seq_len, dtype=bool)
        env_ids[traj_seq_len:] = True  # Environment

        attention_from_tgt = attn[:(1 + obs_len + pred_len), :]
        attention_to_all = attention_from_tgt[:, :]
        # attention_to_all = attn[:, :]
        # attention_to_all[0, :] = 1.0

        attns.append(attention_to_all)

    attns = np.concatenate(attns)

    ax.pcolor(attns, cmap="Reds", vmin=0.0, vmax=0.3, edgecolor="lightgray")
...
def viz_scene_attention(env_patches, env_attentions, layer_id=0, attn_head_id=0, obs_len=8, pred_len=12,
    ...
    traj_seq_len = (obs_len + 1) * (num_nbr + 1) + pred_len
    tgt_ids = np.zeros(traj_seq_len+env_seq_len, dtype=bool)
    tgt_ids[1+obs_len:1+obs_len+pred_len] = True
    for pidx, patch in enumerate(env_patches):
        px = pidx % num_patch
        py = pidx // num_patch
...
def viz_attention_save(axs=None, desc="", path="./output"):
    if axs:
        for ax in axs:
            ax.axis("off")
            title = ax.get_title()
